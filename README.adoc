...
- `create-registration-token`: used to create a registration token
...
| action  | Yes      | create | A valid action among: `create`, `delete`, `create-registration-token`, `skip-testing`.
...
| fully-qualified-names | | false | If the `database_host` and `database_service` output parameters must return a fully qualified hostname and service name (including the domain name): `true` or `false`.
...
| github-token | |  | A valid GitHub token.
| target-repository | | the repository where the action runs | To override the default repository used for registration token creation.
|===
...
| database_protocol          | create   | The protocol to access the database(s).
| database_host              | create   | The hostname of the database(s).
| database_port              | create   | The port of the database(s).
| database_service           | create   | The database service name of the database(s).
| database_user              | create   | The database user of the database(s). If several are provided, then this will be a list of comma-separated users.
...
This project welcomes contributions from the community. Please link:./CONTRIBUTING.md[review our contribution guide].

== Security

Please consult the link:./SECURITY.md[security guide] for our responsible security vulnerability disclosure process.

== License

Copyright (c) 2026 Oracle and/or its affiliates.

Released under the Universal Permissive License v1.0 as shown at link:https://oss.oracle.com/licenses/upl/[].